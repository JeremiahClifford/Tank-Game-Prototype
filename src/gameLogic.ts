...
//target server
const server: string = playerStorage.getItem("Server") as string
//target port
...
const drawBoard = (): void => {
    ...
    fetch(`http://${server}:${port}/players`, {method: "GET"})
       .then(res => res.json())
            ...
            players.forEach((p) => {
                ...
                if (p.PlayerName === playerStorage.getItem("Username")) {
                    ...
                    if (p.Health > 0) { //if the player is alive and thus still in the game
                        ...
                        `
                        const juryBox: HTMLElement = document.getElementById("jury-box") as HTMLElement
                        players.forEach((p) => {
